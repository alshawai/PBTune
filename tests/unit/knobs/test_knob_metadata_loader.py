...
def test_get_knobs_by_tier_fallback_to_expert(caplog):
    module = _load_knob_metadata_module()
    # Ensure not loaded
    ...
    assert "Data-driven tiers requested but not loaded" in caplog.text
...
def test_load_data_driven_tiers_populates_data(tmp_path: Path):
    ...
    assert module.DATA_DRIVEN_TIERS["standard"] == ["work_mem"]
