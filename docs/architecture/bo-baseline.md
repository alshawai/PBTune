...
- Post-hoc cross-method comparison via [`rescore_metrics_globally()`](../../src/utils/calibration.py) pools raw metrics from both PBT and BO runs and rescales using globally-calibrated anchors, so the frozen in-run BO scores are not what the comparison is judged against. See [pbt-vs-bo-comparison guide](../guides/pbt-vs-bo-comparison.md).
