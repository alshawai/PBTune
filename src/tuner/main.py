...
def main() -> int:
    """Deprecated CLI shim → :func:`src.tuners.pbt.cli.main`.

    The PBT execution path now lives in ``src/tuners/pbt`` on the unified
    ``BaseTuner`` framework. This entry point is preserved only so the historic
    invocation keeps working::

        python -m src.tuner.main --tier core --config standard

    New invocations should prefer ``python -m src.tuners.pbt`` (direct) or
    ``python -m src.tuners pbt`` (routed). The legacy :class:`PBTTuner` class
    above is retained for the equivalence test until the PBT test suite is
    relocated (refactor step 2f); it is no longer reachable from this CLI.
    """
    from src.tuners.pbt.cli import main as pbt_cli_main

    LOGGER.warning(
        "%s`python -m src.tuner.main` is deprecated; use "
        "`python -m src.tuners.pbt` (or `python -m src.tuners pbt`).%s",
        COLORS.italic,
        COLORS.reset,
    )
    return pbt_cli_main()
